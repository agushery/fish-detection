...
% Last Modified by GUIDE v2.5 28-Dec-2020 20:34:14
...
% handles    structure with handles and user data (see GUIDATA)
global img_rgb;
[filename pathname]=uigetfile({'*.jpg'}, 'Browse File');
...
% handles    structure with handles and user data (see GUIDATA)
global img_rgb;
img_bw = im2bw(img_rgb, .20);
img_bw =bwareaopen(img_bw,40);
img_bw = imfill(img_bw, 'holes');
se = strel('square', 30);
img = imclose(img_bw, se);
[B,L] = bwboundaries(img,'noholes');
pengukuranIkan = regionprops(L, 'All');
jumlahIkan = length(B);
